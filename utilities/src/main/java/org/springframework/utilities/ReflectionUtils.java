...
public final class ReflectionUtils {
    ...
    public static Object instantiate(Class<?> cls, Object... args)
    ...
    public static Method getMethod(
            Class<?> containingClass, String name, Class<?> returnType, Class<?>[] parameterTypes) {
        Method method = null;
        for (Method declaredMethod : containingClass.getDeclaredMethods()) {
            if (declaredMethod.getName().equals(name)
                    && declaredMethod.getReturnType().equals(returnType)) {
                for (int i = 0; i < declaredMethod.getParameterTypes().length; i++) {
                    if (!parameterTypes[i].equals(declaredMethod.getParameterTypes()[i])) return null;
                }
    ...
    public static <T> List<T> flattenMethodsWith(
            Class<?>[] classes, Class<? extends Annotation> annotationClass, Supplier<Method, T> packer) {
    ...
    public static <T, T1> ArrayList<T1> flattenMethodsWith(
            T[] objects, Class<? extends Annotation> annotationClass, TriSupplier<Method, T, T1> packer) {
