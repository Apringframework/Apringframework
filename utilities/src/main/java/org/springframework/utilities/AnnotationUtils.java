...
public class AnnotationUtils {
     ...
     * @throws IOException
     */
    public List<Class<?>> findClassesWith(ClassLoader classLoader, Class<? extends Annotation> annotationClass)
            throws IOException {
        ArrayList<Class<?>> classes = new ArrayList<>();
        ImmutableSet<ClassPath.ClassInfo> loadedClasses =
                ClassPath.from(classLoader).getAllClasses();
        for (ClassPath.ClassInfo loadedClass : loadedClasses) {
            if (loadedClass.getName().contains("module-info")) continue;
            Class<?> cls = loadedClass.load();
            if (cls.isAnnotationPresent(annotationClass)) classes.add(cls);
        }
    ...
    public List<Method> findMethodsWith(Class<?> containingClass, Class<? extends Annotation> annotationClass) {
        ...
        for (Method method : methods) {
            if (method.isAnnotationPresent(annotationClass)) foundMethods.add(method);
        }
    ...
    public List<Field> findFieldsWith(Class<?> containingClass, Class<? extends Annotation> annotationClass) {
        ...
        for (Field field : fields) {
            if (field.isAnnotationPresent(annotationClass)) foundFields.add(field);
        }
