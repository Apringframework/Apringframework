...
public final class ActivityEventListener implements Listener {
    ...
    public void onActivityStart(ActivityStartEvent event) {
        if (AndroidManager.getDispatcher() == null) return;
        AndroidManager.getDispatcher()
                .dispatch(event.getContext(), new ActivityInitializationAction(event.getName(), event.getActivity()));
    }
}
