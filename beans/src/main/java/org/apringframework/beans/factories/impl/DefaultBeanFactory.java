...
public class DefaultBeanFactory implements HierarchicalBeanFactory {
    ...
    public DefaultBeanFactory(BeanCollector<?> beanCollector, List<BeanInstantiator> instantiators) {
        ...
        this.pendingBeanModels = new ArrayDeque<>();

        if (beanCollector instanceof GroupBeanCollector) {
            GroupBeanCollector groupBeanCollector = (GroupBeanCollector) beanCollector;
            List<BeanCollector<?>> beanCollectors = flattenGroupBeanCollector(groupBeanCollector);
            for (BeanCollector<?> collector : beanCollectors) {
                BeanInstantiator instantiator = instantiators.stream()
                        .filter(i -> i.support(collector))
                        .findAny()
            ...
            BeanInstantiator instantiator = instantiators.stream()
                    .filter(i -> i.support(beanCollector))
                    .findAny()
                    .orElseThrow(() -> new IllegalStateException("Cannot find suitable bean instantiator for "
    ...
    private List<BeanCollector<?>> flattenGroupBeanCollector(GroupBeanCollector root) {
        List<BeanCollector<?>> flatted = new ArrayList<>();

        Deque<GroupBeanCollector> childBeanCollectors = new ArrayDeque<>();
        childBeanCollectors.add(root);

        while (!childBeanCollectors.isEmpty()) {
            GroupBeanCollector current = childBeanCollectors.pop();
            for (BeanCollector<?> beanCollector : current.getBeanModels()) {
                if (beanCollector instanceof GroupBeanCollector) {
                    childBeanCollectors.push((GroupBeanCollector) beanCollector);
                } else flatted.add(beanCollector);
            }
        }
        return flatted;
    }
    ...
    public <T> List<T> getBeansOfType(Class<T> clazz) {
        ...
        if (!beanCache.containsKey(clazz)) {
            // Look up to parent bean factory
            if (getParent() != null) return getParent().getBeansOfType(clazz);

            return null;
        }
        return Collections.singletonList(ObjectUtils.uncheckedCast(beanCache.get(clazz)));
    }
