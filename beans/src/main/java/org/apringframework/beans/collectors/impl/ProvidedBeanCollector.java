 ...
 * @author Singlerr
 */
public final class ProvidedBeanCollector implements BeanCollector<MethodInfo> {

    private final List<Class<?>> targetClasses;

    private Class<? extends Annotation> targetAnnotation = Bean.class;

    private List<MethodInfo> collectedMethods;
    ...
    private void initialize() {
        ...
        for (Class<?> targetClass : targetClasses) {
            collectedMethods.addAll(AnnotationUtils.findMethodsWith(targetClass, targetAnnotation).stream()
                    .map(m -> new MethodInfo(m, targetClass))
     ...
     */
    @Override
    public List<MethodInfo> getBeanModels() {
        if (collectedMethods == null) initialize();
